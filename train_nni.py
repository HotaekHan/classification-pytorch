...
config['params']['batch_size'] = tuner_params['batch_size']
config['optimizer']['lr'] = tuner_params['lr']
config['params']['mixup_alpha'] = tuner_params['mixup']
config['params']['label_smoothing'] = tuner_params['smoothing']

# '''make output folder'''
# if not os.path.exists(config['exp']['path']):
#     os.mkdir(config['exp']['path'])
#
...
if __name__ == '__main__':
    for epoch in range(start_epoch, 100, 1):
        ...
        loss = iterate(epoch=epoch, phase='valid')
